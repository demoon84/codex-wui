(() => {
  ...
  internals.invoke = async (cmd, args = {}) => {
    ...
    switch (cmd) {
      ...
      case 'set_model':
        settings.model = firstDefined(args.model_id, args.modelId, 'gpt-5-codex');
        saveSettings(settings);
      ...
      case 'switch_workspace':
        return { success: true, sessionId: `mock-${firstDefined(args.workspace_id, args.workspaceId, 'session')}` };
      case 'stream_codex': {
        const cid = firstDefined(args.conversation_id, args.conversationId, '');
      ...
      case 'db_create_workspace': {
        ...
        const workspacePath = firstDefined(args.workspace_path, args.workspacePath, '');
        ...
        const workspace = state.workspaces.find((w) => w.id === args.id);
        return workspace || { id: args.id, name: args.name, path: workspacePath, conversations: [] };
      }
      ...
      case 'db_get_conversations': {
        const state = loadState();
        const workspace = ensureWorkspace(state, firstDefined(args.workspace_id, args.workspaceId));
        return workspace?.conversations || [];
      }
      case 'db_create_conversation': {
        const state = loadState();
        const workspaceId = firstDefined(args.workspace_id, args.workspaceId);
      ...
      case 'db_get_messages': {
        const state = loadState();
        const conversationId = firstDefined(args.conversation_id, args.conversationId);
        for (const workspace of state.workspaces) {
          const conv = ensureConversation(workspace, conversationId);
